...
const Navbar = (props) => {
  ...
  const [state, setState] = useState({ order: "", customer: "" });
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          <ul className="navbar-nav mr-auto">
            <li className="nav-item">
              <Link className="nav-link" to="/editor/home">
                Home <span className="sr-only">(current)</span>
              </Link>
            </li>
            <li className="nav-item">
              <Link className="nav-link" to="/editor/create-order">
                Create Order
              </Link>
            </li>

            <li className="nav-item">
              <Link className="nav-link " to="/editor/create-account">
                Create Account
              </Link>
            </li>
          </ul>
          <form
            onSubmit={(e) => {
              ...
              push(
                `/editor/search/orders-by-title?titleOfOrder=${state.order}`
              );
              setState({ order: "", customer: "" });
            }}
          ...
          <form
            onSubmit={(e) => {
              ...
              push(
                `/editor/search/customers-by-office-name?officeName=${state.customer}`
              );
              setState({ order: "", customer: "" });
            }}
            className="form-inline my-2 my-lg-0 second-input"
          >
            <input
              ...
              className="form-control mr-sm-2"
              type="search"
              placeholder="Search Customer(officename)"
              aria-label="Search Customer(officename)"
            />
          </form>
          <div className="btn-group account" role="group">
            ...
            <div className="dropdown-menu" aria-labelledby="btnGroupDrop1">
              <Link className="dropdown-item" to="/editor/profile">
                <svg
                  aria-hidden="true"
                  focusable="false"
                  data-prefix="far"
              ...
              <Link
                ...
                to="#"
                onClick={() => {
                  LOGOUT({ route: "/editor/logout", IS_LOADING, push });
                }}
              >
                <svg
                  aria-hidden="true"
